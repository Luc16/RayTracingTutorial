#ifndef CONSTANT_MEDIUM_H
#define CONSTANT_MEDIUM_H

#include "../../rttutorial.h"
#include "../../generals/hittable.h"
#include "../materials/isotropic.h"

class constant_medium: public hittable {
public:
    constant_medium(shared_ptr<hittable> obj, double d, shared_ptr<texture> tex):
    boundary(std::move(obj)), phase_function(make_shared<isotropic>(std::move(tex))), negative_inv_density(-1/d) {}

    constant_medium(shared_ptr<hittable> obj, double d, color c):
            boundary(std::move(obj)), phase_function(make_shared<isotropic>(c)), negative_inv_density(-1/d) {}

    bool hit(const ray &r, double t_min, double t_max, hit_record &rec) const override;
    bool bounding_box(double time0, double time1, aabb &output_box) const override;


private:
    shared_ptr<hittable> boundary;
    shared_ptr<material> phase_function;
    double negative_inv_density;
};

bool constant_medium::hit(const ray &r, double t_min, double t_max, hit_record &rec) const {
    const bool enable_debugging = false;
    const bool debugging = enable_debugging && random_double() < 0.00001;

    hit_record rec1, rec2;

    if (!boundary->hit(r, -infinity, infinity, rec1)) return false;
    if (!boundary->hit(r, rec1.t + 0.0001, infinity, rec2)) return false;

    if (debugging) std::cout << "\nt_min=" << rec1.t << ", t_max=" << rec2.t << '\n';

    if (rec1.t < t_min) rec1.t = t_min;
    if (rec2.t > t_max) rec2.t = t_max;

    if (rec1.t >= rec2.t) return false;
    if (rec1.t < 0) rec1.t = 0;

    const auto ray_length = r.direction().length();
    const auto distance_inside_boundary = (rec2.t - rec1.t)/ray_length;
    const auto hit_distance = negative_inv_density * log(random_double());

    if (hit_distance > distance_inside_boundary) return false;

    if (debugging) {
        std::cout << "hit_distance = " <<  hit_distance << '\n'
                  << "rec.t = " <<  rec.t << '\n'
                  << "rec.p = " <<  rec.p << '\n';
    }

    rec.normal = vec3(1, 0, 0);
    rec.front_face = true;
    rec.mat_ptr = phase_function;


}

bool constant_medium::bounding_box(double time0, double time1, aabb &output_box) const {
    return boundary->bounding_box(time0, time1, output_box);
}

#endif //CONSTANT_MEDIUM_H
